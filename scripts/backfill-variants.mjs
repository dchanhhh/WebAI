...
async function main() {
  // Dùng SQL thô để đọc `stock` vì cột này có thể đã bị xoá khỏi schema.prisma
  // hiện tại (script chỉ hữu ích ở giai đoạn chuyển tiếp: bảng ProductVariant
  // đã tồn tại nhưng cột Product.stock vật lý trong DB chưa bị DROP).
  const products = await prisma.$queryRawUnsafe(
    "SELECT id, name, slug, stock, sizes, colors FROM Product ORDER BY createdAt ASC",
  );
